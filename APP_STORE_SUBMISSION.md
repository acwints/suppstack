...
The repo contains a complete Capacitor iOS shell (`ios/`) that wraps the
production web app. Everything that can be prepared from code is done; this
guide covers the remaining steps, which require an Apple Developer account and
a Mac with Xcode.

## How the app is built

- `capacitor.config.ts` points the native WKWebView at the production site
  (`https://suppstack.vercel.app`). The web app is SSR + API routes, so it is
  served remotely rather than bundled. Shipping web updates does NOT require
...
- Native integrations are bridged into the remote page by Capacitor:
  ...
  - Splash screen and status bar are configured in `capacitor.config.ts`;
    icons/splash assets live in `ios/App/App/Assets.xcassets` (regenerate with
    `npm run ios:assets` from `assets/`).
- `native/shell/index.html` is only an offline fallback page.
